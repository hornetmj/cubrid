...
do_create_entity (PARSER_CONTEXT *parser, PT_NODE *node)
{
  ...
  switch (node->info.create_entity.entity_type)
      ...
      if (tbl_opt_replication)
	{
	  ...
	  assert (replication_node != NULL && replication_node->node_type == PT_VALUE
		  && replication_node->type_enum == PT_TYPE_INTEGER);
	  repl_opt = replication_node->info.value.data_value.i;
